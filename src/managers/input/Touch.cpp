...
void CInputManager::onTouchDown(ITouch::SDownEvent e) {
    ...
    if (g_pSessionLockManager->isSessionLocked()) {
        ...
        Debug::log(WARN, "going to send touch down to lock surface");
        if (m_sTouchData.touchFocusLockSurface == nullptr) {
            Debug::log(WARN, "can't find lock surface?");
        } else {
            m_sTouchData.touchFocusSurface = m_sTouchData.touchFocusLockSurface->surface->surface();
        }
    } else {
        m_sTouchData.touchFocusLockSurface = nullptr;
